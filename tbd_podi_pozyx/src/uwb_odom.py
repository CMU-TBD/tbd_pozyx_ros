...
class uwbOdom:
    def __init__(self, base_frame = "uwb", robot_name = "robot"):
        ...
        if(rospy.get_param(self.name + '/use_cov_param')):
            self.pos_orient_cov = np.diag([rospy.get_param(self.name + '/pose_cov')])

            self.ang_vel_cov = np.diag([rospy.get_param(self.name + '/ang_cov')])

            self.accel_cov = np.diag([rospy.get_param(self.name + '/accel_cov')])
        else:
